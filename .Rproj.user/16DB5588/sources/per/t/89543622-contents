...
media_valor <- mean(dados$VPL10)
media_valor

dp_valor <- sd(dados$VPL10)
dp_valor
...
GVPL10 <- ggplot(dados, aes(x = VPL10)) +
  ...
  geom_area(
    ...
    args = list(mean = media_valor, sd = dp_valor),
    fill = "red", alpha = 0.5,
    xlim = c(valor_referencia, min(dados$VPL10))
  ) +
  labs(
    x = "VPL até 10º Ano (mil R$)",
    y = "Densidade"
  ) +
  annotate(
    "text", x = -3000000, y = 3e-8,
    label = paste0("Prob = ", round(probabilidade * 100, 2), "%"),
    color = "red", size = 3.9, fontface = "bold"
  ) +
  annotate(
    "text", x = 15000000, y = 8e-8,
...
# Média e DP para VPL10
media_valor <- mean(dados$TER10)
media_valor

dp_valor <- sd(dados$TER10)
dp_valor
...
# Gerar o gráfico ajustado
GTER10 <- ggplot(dados, aes(x = TER10)) +
  geom_histogram(aes(y = ..density..), bins = 20, fill = "blue", color = "black", alpha = 0.7)+
  geom_vline(xintercept = media_valor, linetype = "dashed", color = "black", size = 1) +
